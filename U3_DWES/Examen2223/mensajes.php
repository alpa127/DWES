...
//Chequear si hay empleado en la sesión
	...
	if(isset($_POST['Enviar'])){
		if(empty($_POST['asunto']) or empty($_POST['mensaje'])){
			...
			if($id!=0){
				$mensaje = 'Mensaje nº'.$m->getIdMen().' enviado'.$id;
			}else{
...
<html>
     ...
     <body>
        	...
        	<form action="mensajes.php" method="post">              	
            		<h1 style="color:blue;"><?php echo isset($mensaje)?$mensaje:'';?></h1> 
            		<h2 style="color:blue;">
				<?php
				echo 'Nombre:'.$empleado->getNombre().'-'.
				'DNI:'.$empleado->getDni().'- Departamento'.$empleado->getDepartamento()->getNombre(); 
				?></h2>             		
            		<hr/> 
            		<div> 
            		...
            		<table width="100%">
						...
						<?php
						$mensajes = $bd->obtenerMensajeRecibidos($empleado);
						foreach($mensajes as $m){
							echo '<tr>';
            				echo '<th align="left">'.$m->getIdMen().'</th>';
            				echo '<th align="left">'.$m->getDeEmpleado()->getNombre().'</th>';
							echo '<th align="left">'.$m->ParaDepartamento()->getNombre().'</th>';
            				echo '<th align="left">'.
							date('d/m/Y',strtotime($m->getFechaEnvio())).'</th>';
            		...
            		<table width="100%">
						...
						<?php
						$mensajes = $bd->obtenerMensaje($empleado);
						foreach($mensajes as $m){
							echo '<tr>';
            				echo '<th align="left">'.$m->getIdMen().'</th>';
