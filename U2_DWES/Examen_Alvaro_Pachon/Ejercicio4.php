...
<head>
    ...
    <title>Ejercicio4_Alvaro_Pachon</title>
</head>
<body>
    <h1>MORALIA CF</h1>
        ...
        if(isset($_POST['enviar'])){
           ...
           !isset($_POST['categoria']) or !isset($_POST['tipoC']) or !isset($_POST['competiciones']) or
            ...
            $jugador = new Jugador(
                ...
                $_POST['categoria'],
                $_POST['tipoC'],
                implode(',',$_POST['competiciones']),
                implode(',',$_POST['extras'])
            );
            if ($mod->crearJugadores($jugador)) {
                echo '<h3 style="color:blue">Jugador creado</h3>';
            } else {
                echo '<h3 style="color:red">Error al crear el jugador</h3>';
            }
        }
    }
    $jugador = $mod->obtenerJugadores();
    if(isset($_POST['ver'])){
    ?>
    <table border= 3  width="50%" align="center" style="text-align:center;border-collapse:collapse;padding: 15px;">
        <tr style="background-color: aquamarine;">
            <td><b>Número del jugador</b></td>
        ...
        <?php
            foreach($jugador as $j){
                ...
                echo '<td>'.$j->getFecha().'</td>';
                echo '<td>'.$j->getCategoria().'</td>';
                echo '<td>'.$j->getTipoC().'</td>';
                echo '<td>'.$j->getCompeticiones().'</td>';
